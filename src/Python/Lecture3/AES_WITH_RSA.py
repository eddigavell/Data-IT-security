...
def generate_keys():
    ...
    while True:
        passphrase = input('Enter password for private key: ')
        passphrase_repeat = input('Please repeat the password: ')
...
def encrypt_message():
    ...
    value = [encrypted_session_key, aes_cipher.nonce, tag, cipher_text]
...
def decrypt_message():
    ...
    passprase = input('Enter the passphrase used for this key: ')

    private_key = read_private_key(private_key_name, passprase)
    with open(f'{file_name}.enc', 'rb') as in_file:
        enc_session_key, nonce, tag, cipher_text = [in_file.read(b) for b in
    ...
    print("The decrypted message is: " + message.decode('utf-8'))
